...
class Controller_Index extends Profile
{
	...
	function action_index()
    {
		...
		/*
		$config = $this->config = loadConfig('shop_config');
		$client_data = false;
		$gid = $this->user->getGroupID();
		if (isset($config['paid_groups'][$gid]))
		{
			$uid = $this->user->get('uid');
			$clients = new Model_Shop_Clients();
			$def = $config['user_ads_counts'][$gid];
			...
			if ($client_data['orders'] == false)
			{
				...
				{
					$client_data['count'] = $def['count'];
				}
			}
			else 
			{
				$client_data['count'] = $client['orders'][0]['count_ad'];
			}
		}
