...
class Debug 
{
		...
		elseif (is_resource($var))
		{
			if (($type = get_resource_type($var)) === 'stream' AND $meta = stream_get_meta_data($var))
			{
				...
				if (isset($meta['uri']))
				{
					...
					return '<small>resource</small><span>('.$type.')</span> '.htmlspecialchars($file, ENT_NOQUOTES, 'UTF8');
				}
			}
			else
			{
	...
	public static function path($file)
	{
		...
		elseif (strpos($file, MAINPATH) === 0)
		{
			$file = 'MAINPATH'.DIRECTORY_SEPARATOR.substr($file, strlen(MAINPATH));
		}
