	...
	<div class="index_list">
		...
		?>
			<tr class="trow<?=$even;?> forum_depth">
				<td class="trow<?=$even;?>" align="center" valign="top" width="1"><img src="/forum/images/on.gif" ></td>
				
				<td valign="top" class="category_item trow<?=$even;?> forum_depth">				
					<?php
					
					if (isset($cat['subcategories']) && $cat['subcategories'])
					{
					?>
					<strong class="category_name"><a href="<?=Request::$base_url;?>shop/subcategory/<?=$cat['id'];?>" title="<?=$cat['title'];?>"><?=$cat['title'];?></a></strong>
				...
				<td class="trow<?=$even;?> forum_depth">
					<?=$cat['ad_counts'];?>
				</td>
				<td class="last_cat_info trow<?=$even;?> forum_depth" align="left">
				    <?php
						...
						?>
							<div class="img-preview">
								<a href="<?=Request::$base_url;?>shop/view/<?=$cat['last_data']['last_ad_id'];?>">
									<img src="/<?=is_file(DOCROOT.$cat['last_data']['last_ad_image'])?$cat['last_data']['last_ad_image']:'img/shop/no-image.png" width="79px" height="53px';?>" />
								</a>
							</div>
					</td>
					<td class="trow<?=$even;?> forum_depth last_cat_info">
						...
						<span class="smalltext"><?=View::format_date($cat['last_data']['last_ad_date']);?></span><br />
						<span class="smalltext">Продавец:</span> <a href="/profile/<?=$cat['last_data']['last_ad_uid'];?>" class="username"><?=$cat['last_data']['last_ad_username'];?></a>
