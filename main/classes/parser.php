...
class Parser
{
	...
	function parse_message($message, $options=array())
	{
		...
		if(isset($options['nl2br']) && $options['nl2br'] !== 0)
		{
			...
			$message = preg_replace("#(</?(?:html|head|body|div|p|form|table|thead|tbody|tfoot|tr|td|th|ul|ol|li|div|p|hr)[^>]*>)\s*<br />#i", "$1", $message);
			$message = preg_replace("#(&nbsp;)+(</?(?:html|head|body|div|p|form|table|thead|tbody|tfoot|tr|td|th|ul|ol|li|div|p|hr)[^>]*>)#i", "$2", $message);
		}
		
		$message = $this->cut_string($message, 700);
		return $message;
	}
	...
	function parse_mycode($message, $options=array())
	{
		...
		if(isset($options['allow_videocode']) && $options['allow_videocode'] != 0)
		{
	...
	private function cache_smilies()
	{
		...
		$smilies = $smilies = DB::query('SELECT * FROM mybb_smilies WHERE showclickable = 1 ORDER BY disporder ASC')->fetchAll();
	...
	function mycode_parse_quotes($message, $text_only=false)
	{
		...
		if($text_only == false)
		{
			...
			$replace = array(
				...
				"<blockquote><cite>Цитата</cite>$1</blockquote>"
			);
		}
		else
		{
	...
	*/
	function mycode_parse_post_quotes($message, $username, $text_only=false)
	{
		...
		if(isset($match[1]) && intval($match[1]))
		{
		...
		if(isset($match[1]) && intval($match[1]))
		{
			if($match[1] < TIME_NOW)
			{
				$date = View::format_date(intval($match[1]));
			}
		...
		if($delete_quote)
		{
			$username = mb_substr($username, 0, mb_strlen($username)-1);
		}
		...
		{
			//throw new NException('dump');
			return '<blockquote><cite style="background: #eee;">'.$username."</cite><i>".$this->cut_string($message, 200)."</i></blockquote>";
		}
	}
	...
	*/
	function mycode_parse_url($url, $name="")
	{
		...
		if($name == $url && (!isset($this->options['shorten_urls']) || $this->options['shorten_urls'] != 0))
		{
			if(mb_strlen($url) > 55)
			{
				$name = mb_substr($url, 0, 40)."...".mb_substr($url, -10);
			}
		}
	...
	function mycode_parse_img($url, $dimensions=array(), $align='')
	{
		...
		if(my_strlen($alt) > 55)
		{
			$alt = mb_substr($alt, 0, 40)."...".mb_substr($alt, -10);
		}
